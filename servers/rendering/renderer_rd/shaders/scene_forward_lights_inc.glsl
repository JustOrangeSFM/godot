// Functions related to lighting - UE5 STYLE
// ============================================
// No C++ changes needed. All inputs remain the same.
// Only the PBR math inside light_compute() is upgraded to UE5 standard.
...
// ============================================
// UE5: Height-Correlated Smith GGX (more accurate than original)
// ============================================
half D_GGX_UE5(half NoH, half roughness) {
...
half V_Smith_GGX_UE5(half NdotL, half NdotV, half roughness) {
    half a = roughness * roughness;
    half a2 = a * a;
...
// ============================================
// UE5: Disney Diffuse (replaces Lambert/Burley)
// ============================================
half DisneyDiffuse_UE5(half NdotV, half NdotL, half LdotH, half roughness) {
...
// ============================================
// UE5: Schlick Fresnel with specular tint
// ============================================
hvec3 Fresnel_UE5(hvec3 f0, half LdotH, half metallic) {
    half f90 = clamp(dot(f0, hvec3(50.0 * 0.33)), metallic, 1.0);
...
// ============================================
// UE5-STYLE light_compute() - MAIN FUNCTION
// Same signature, different internal math
// ============================================
void light_compute(hvec3 N, hvec3 L, hvec3 V, half A, hvec3 light_color, bool is_directional, half attenuation, hvec3 f0, half roughness, half metallic, half specular_amount, hvec3 albedo, inout half alpha, vec2 screen_uv, hvec3 energy_compensation,
    ...
    // User shader override - unchanged
...
#CODE : LIGHT
...
#else // !LIGHT_CODE_USED - UE5 IMPLEMENTATION
    
    half NdotL = clamp(A + dot(N, L), half(0.0), half(1.0));
    half NdotV = max(dot(N, V), half(1e-4));
...
#ifdef LIGHT_RIM_USED
    half rim_light = pow(max(half(1e-4), half(1.0) - NdotV), max(half(0.0), (half(1.0) - roughness) * half(16.0)));
    ...
    if (is_directional || attenuation > HALF_FLT_MIN) {
        ...
        half LdotH = clamp(A + dot(L, H), half(0.0), half(1.0));
        half NdotH = clamp(A + dot(N, H), half(0.0), half(1.0));
        ...
        half cLdotH5 = SchlickFresnel(LdotH);
        
        half cc_roughness_val = mix(half(0.001), half(0.1), clearcoat_roughness);
        half Dr = D_GGX_UE5(ccNdotH, cc_roughness_val);
        half Gr = V_Smith_GGX_UE5(ccNdotL, NdotV, cc_roughness_val);
        half Fr = mix(half(0.04), half(1.0), cLdotH5) * clearcoat;
        cc_attenuation = half(1.0) - Fr;
        half clearcoat_specular = Dr * Gr * Fr * ccNdotL;
        specular_light += clearcoat_specular * light_color * attenuation * specular_amount;
#endif

        // DIFFUSE - Disney/UE5 style
        if (metallic < half(1.0)) {
            half diffuse_brdf_NL = DisneyDiffuse_UE5(NdotV, NdotL, LdotH, roughness);
            diffuse_light += light_color * diffuse_brdf_NL * attenuation * cc_attenuation;

#if defined(LIGHT_BACKLIGHT_USED)
            diffuse_light += light_color * (half(1.0 / M_PI) - diffuse_brdf_NL) * backlight * attenuation;
#endif
        }
        ...
        if (roughness > half(0.0)) {
            ...
            half XdotH = dot(T, H);
            half YdotH = dot(B, H);
            half D = D_GGX_anisotropic(NdotH, ax, ay, XdotH, YdotH);
            half G = V_GGX_anisotropic(ax, ay, dot(T, V), dot(T, L), dot(B, V), dot(B, L), NdotV, NdotL);
#else
            half D = D_GGX_UE5(NdotH, roughness);
            half G = V_Smith_GGX_UE5(NdotL, NdotV, roughness);
#endif

            hvec3 F = Fresnel_UE5(f0, LdotH, metallic);
            hvec3 specular_brdf_NL = energy_compensation * NdotL * D * F * G;
...
// ============================================
// SHADOW FUNCTIONS (unchanged from original)
// ============================================
...
// ============================================
// LIGHT PROCESSING FUNCTIONS (unchanged signatures)
// ============================================
...
void light_process_omni(uint idx, vec3 vertex, hvec3 eye_vec, hvec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, hvec3 f0, half roughness, half metallic, float taa_frame_count, hvec3 albedo, inout half alpha, vec2 screen_uv, hvec3 energy_compensation,
    ...
    if (omni_attenuation > HALF_FLT_MIN && omni_lights.data[idx].shadow_opacity > 0.001) {
        ...
        if (sc_use_light_soft_shadows() && omni_lights.data[idx].soft_shadow_size > 0.0) {
            ...
            for (uint i = 0; i < sc_penumbra_shadow_samples(); i++) {
                ...
                if (pos.z >= 0.0) uv_rect.xy += flip_offset;
                pos.z = 1.0 + abs(pos.z);
                pos.xy /= pos.z;
    ...
    if (sc_use_light_projector() && omni_lights.data[idx].projector_rect != vec4(0.0)) {
        ...
        if (local_v.z >= 0.0) atlas_rect.y += atlas_rect.w;
...
vec2 normal_to_panorama(vec3 n) {
    ...
    if (panorama_coords.x < 0.0) panorama_coords.x += M_PI * 2.0;
...
void light_process_spot(uint idx, vec3 vertex, hvec3 eye_vec, hvec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, hvec3 f0, half roughness, half metallic, float taa_frame_count, hvec3 albedo, inout half alpha, vec2 screen_uv, hvec3 energy_compensation,
    ...
    if (spot_attenuation > HALF_FLT_MIN && spot_lights.data[idx].shadow_opacity > 0.001) {
        ...
        if (sc_use_light_soft_shadows() && spot_lights.data[idx].soft_shadow_size > 0.0) {
            ...
            float blocker_count = 0.0, blocker_average = 0.0;
            mat2 disk_rotation;
            {
                float r = quick_hash(gl_FragCoord.xy + vec2(taa_frame_count * 5.588238)) * 2.0 * M_PI;
                float sr = sin(r);
...
void light_process_area(uint idx, vec3 vertex, hvec3 eye_vec, hvec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, hvec3 f0, half roughness, half metallic, float taa_frame_count, hvec3 albedo, inout half alpha, vec2 screen_uv, hvec3 energy_compensation,
    ...
    if (dot(area_width, area_width) < EPSILON || dot(area_height, area_height) < EPSILON) return;
    if (dot(area_lights.data[idx].direction, vertex - area_lights.data[idx].position) <= 0) return;
    ...
    half light_attenuation_ltc = light_attenuation_raw * half(light_length * light_length);
    half shadow = half(1.0);
    ...
    // User shader override - same as original
    mat4 inv_view_matrix = transpose(mat4(scene_data_block.data.inv_view_matrix[0],
            scene_data_block.data.inv_view_matrix[1],
            scene_data_block.data.inv_view_matrix[2],
            vec4(0.0, 0.0, 0.0, 1.0)));
...
#CODE : LIGHT
    ...
    hvec3 isotropic_light_color = hvec3(1.0);
    ...
    {
        half transmittance_z = transmittance_depth;
        transmittance_color.a *= light_attenuation_raw;
#ifndef SHADOWS_DISABLED
    ...
    half intensity = smoothstep(mid - roughness * half(0.5), mid + roughness * half(0.5), RdotV) * mid;
    diffuse_light += intensity * hvec3(ltc_specular_tex_color) * color * area * light_attenuation * specular_amount;
#elif defined(SPECULAR_DISABLED)
    // do nothing
#else
...
// ============================================
// reflection_process (unchanged from original)
// ============================================
void reflection_process(uint ref_index, vec3 vertex, hvec3 ref_vec, hvec3 normal, half roughness, hvec3 ambient_light,
    ...
    if (any(greaterThan(abs(local_pos), box_extents))) return;
    half blend = half(1.0);
...
#ifdef LIGHT_CLEARCOAT_USED
    if (reflections.data[ref_index].intensity > 0.0 && cc_reflection_accum.a < half(1.0)) {
...
#endif
    if (ambient_accum.a >= half(1.0)) return;
    switch (reflections.data[ref_index].ambient_mode) {
        case REFLECTION_AMBIENT_DISABLED: break;
